...
_app.constants = {
	...
	studyLevel: {
		...
		master: {
			value: 2,
			text: "Posgrado-Maestría"
		},
		doctorate: {
			value: 3,
			text: "Posgrado-Doctorado"
		},
		secondSpeciality: {
			value: 4,
			text: "Posgrado-Segunda Especialidad"
		},
		diplomat: {
			value: 5,
			text: "Posgrado-Diplomado"
		},
		values: [
			{ value: 1, text: "Pregrado" },
			{ value: 2, text: "Posgrado-Maestría" },
			{ value: 3, text: "Posgrado-Doctorado" },
			{ value: 4, text: "Posgrado-Segunda Especialidad" },
			{ value: 5, text: "Posgrado-Diplomado" }
		]
	},
	representativeType: {
		none: {
...
$.fn.datepicker.dates.es = {
	...
	format: _app.constants.formats.datepicker,//"dd/mm/yyyy",
	months: monthNames,
...
$.fn.datetimepicker.defaults.templates = {
    leftArrow: "<i class=\"la la-angle-left\"></i>",
    rightArrow: "<i class=\"la la-angle-right\"></i>"
};
...
(function ($) {
	...
	$.fn.blockLoader = function (message = "Cargando...") {
		mApp.block($(this), { type: "loader", message: message });
		return this;
	};
...
jQuery.validator.setDefaults({
	errorPlacement: function (error, element) {
		if (element.parent(".input-group").length) {
		    ...
		    //let el = element.closest(".m-form__group-sub, .m-form__group");
		    //var classList = [];
		    //$.each($(el).get(0).classList, function(index, value) {
		    //    classList.push(value);
		    //});
		    //$.each(classList, function(index, value) {
		    //    $(el).get(0).classList.remove(value);
		    //});
		    //$(el).get(0).classList.add("has-danger");
		    //$.each(classList, function(index, value) {
		    //    if(value !== "m-form__group-sub")
		    //        $(el).get(0).classList.add(value);
		    //});
		    error.addClass("text-danger").insertAfter(element.parent().find(".note-editor"));
		}
		else {
